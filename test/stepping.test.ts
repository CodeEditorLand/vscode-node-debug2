...
suite("Stepping", () => {
	...
	suite("basic", () => {
		...
		test("returns the same frameIDs between steps", async () => {
			...
			firstFrameIDs = stackTraceResponse.body.stackFrames.map(
				(frame) => frame.id
			);
			await dc.nextAndStop();
			const stackTraceResponse2 = await dc.stackTraceRequest();
			const secondFrameIDs = stackTraceResponse2.body.stackFrames.map(
				(frame) => frame.id
			);
	...
	suite("skipFiles", () => {
		test("steps through un-sourcemapped file", async () => {
			const program = path.join(
				DATA_ROOT,
				"sourcemaps-with-and-without/out/mapped.js"
			);
			const programSource = path.join(
				DATA_ROOT,
				"sourcemaps-with-and-without/src/mapped.ts"
			);

			const skipFiles = ["unmapped"];

			await dc.hitBreakpoint(
				{ program, sourceMaps: true, skipFiles },
				{ path: programSource, line: 7 }
			);
		...
		test("steps through sourcemapped file", async () => {
			const program = path.join(
				DATA_ROOT,
				"calls-between-sourcemapped-files/out/sourceA.js"
			);
			const programSource = path.join(
				DATA_ROOT,
				"calls-between-sourcemapped-files/src/sourceA.ts"
			);
		...
		test("steps over exception in skipped file", () => {
			const program = path.join(
				DATA_ROOT,
				"calls-between-files-with-exception/out/sourceA.js"
			);
			const programSource = path.join(
				DATA_ROOT,
				"calls-between-files-with-exception/src/sourceA.ts"
			);
		...
		test("still stops at breakpoint in skipped file", async () => {
			...
			const programASource = path.join(
				DATA_ROOT,
				"calls-between-sourcemapped-files/src/sourceA.ts"
			);
			const programBSource = path.join(
				DATA_ROOT,
				"calls-between-sourcemapped-files/src/sourceB.ts"
			);
			...
			await dc.hitBreakpoint(
				{ program, sourceMaps: true, skipFiles },
				{ path: programASource, line: 8 }
			);
			await dc.setBreakpointsRequest({
				source: { path: programBSource },
				breakpoints: [{ line: bpLineB }],
			});
			await dc.stepInTo("breakpoint", {
				path: programBSource,
				line: bpLineB,
			});
		});
		...
		test("can toggle skipping on and off", async () => {
			...
			await Promise.all([
				dc
					...
					.then(() =>
						dc.setBreakpointsRequest({
							source: { path: programASource },
							breakpoints: [{ line: bpLineA }],
						})
					)
					.then(() => dc.configurationDoneRequest()),
			...
			await dc.continueTo("breakpoint", {
				path: programASource,
				line: bpLineA,
			});
		...
		test("when generated script is skipped via regex, the source can be un-skipped", async () => {
			const program = path.join(
				DATA_ROOT,
				"calls-between-merged-files/out/sourceA.js"
			);
			const sourceA = path.join(
				DATA_ROOT,
				"calls-between-merged-files/sourceA.ts"
			);
			const sourceB2 = path.join(
				DATA_ROOT,
				"calls-between-merged-files/sourceB2.ts"
			);
		...
		test("when a non-sourcemapped script is skipped via regex, it can be unskipped", async () => {
			...
			const sourceB = path.join(
				DATA_ROOT,
				"calls-between-sourcemapped-files/out/sourceB.js"
			);
			...
			const bpLineA = 7;
			await dc.hitBreakpoint(
				{ program, sourceMaps: false, skipFiles },
				{ path: program, line: bpLineA }
			);
		...
		test("when multiple generated scripts are skipped via one regex, one source can be un-skipped and re-skipped", async () => {
			const program = path.join(
				DATA_ROOT,
				"calls-between-multiple-files/sourceA.ts"
			);
			const sourceB2 = path.join(
				DATA_ROOT,
				"calls-between-multiple-files/sourceB2.ts"
			);
			...
			const outFiles = [
				path.join(
					DATA_ROOT,
					"calls-between-multiple-files/out/**/*.js"
				),
			];
			await dc.hitBreakpoint(
				{ program, skipFiles, outFiles },
				{ path: program, line: bpLineA }
			);
		...
		test("can skip node internal files using <node_internals>", async () => {
			...
			const internalsFrames = stackTraceResponse.body.stackFrames.filter(
				(frame) =>
					frame.source &&
					frame.source.path &&
					frame.source.path.startsWith("<node_internals>/")
			);
			assert(internalsFrames.length > 1);
			internalsFrames.forEach((frame) =>
				assert.equal(
					(<any>frame.source).presentationHint,
					"deemphasize"
				)
			);
