...
suite("Breakpoints", () => {
	...
	suite("setBreakpoints", () => {
		...
		test("should stop on a breakpoint identical to the entrypoint", () => {
			// verifies the 'hide break on entry point' logic
			const PROGRAM = path.join(DATA_ROOT, "program.js");
			const ENTRY_LINE = 1;
		...
		test("should stop on a conditional breakpoint", () => {
			...
			const bp: DebugProtocol.SourceBreakpoint = {
	...
	suite("setBreakpoints in TypeScript", () => {
		test("should stop on a breakpoint in source (all files top level)", () => {
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemaps-simple/classes.js"
			);
			const TS_SOURCE = path.join(
				DATA_ROOT,
				"sourcemaps-simple/classes.ts"
			);
			const TS_LINE = 17;
		...
		test.skip("should stop on a breakpoint in source (all files top level, missing sourceMappingURL)", () => {
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemaps-simple-no-sourceMappingURL/classes.js"
			);
			const TS_SOURCE = path.join(
				DATA_ROOT,
				"sourcemaps-simple-no-sourceMappingURL/classes.ts"
			);
			const TS_LINE = 17;
		...
		test("should stop on a breakpoint in source (outDir)", () => {
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemaps-inline/src/classes.ts"
			);
		...
		test("should stop on a breakpoint in source (outFiles)", () => {
			...
			const OUT_FILES = path.join(
				DATA_ROOT,
				"sourcemaps-inline/dist/**/*.js"
			);
			const BREAKPOINT_LINE = 17;
		...
		test("should stop on a breakpoint in source with spaces in paths (outDir)", () => {
			...
			const OUT_DIR = path.join(
				DATA_ROOT,
				"sourcemaps with spaces/the distribution"
			);
			const BREAKPOINT_LINE = 17;
		...
		test("should stop on a breakpoint in source with spaces in paths (outFiles)", () => {
			...
			const OUT_FILES = path.join(
				DATA_ROOT,
				"sourcemaps with spaces/the distribution/**/*.js"
			);
			const BREAKPOINT_LINE = 17;
		...
		test("should stop on a breakpoint in source - Microsoft/vscode#2574", () => {
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemaps-2574/out/classes.js"
			);
			const OUT_DIR = path.join(DATA_ROOT, "sourcemaps-2574/out");
			const TS_SOURCE = path.join(
				DATA_ROOT,
				"sourcemaps-2574/src/classes.ts"
			);
			const TS_LINE = 17;
		...
		test.skip("should stop on a breakpoint in source (sourceMappingURL missing)", () => {
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemap-no-sourceMappingURL/out/classes.js"
			);
			const OUT_DIR = path.join(
				DATA_ROOT,
				"sourcemap-no-sourceMappingURL/out"
			);
			const TS_SOURCE = path.join(
				DATA_ROOT,
				"sourcemap-no-sourceMappingURL/src/classes.ts"
			);
			const TS_LINE = 17;
		...
		test("should stop on a breakpoint when the sourcemap is loaded after the bp is set", () => {
			...
			return Promise.all<DebugProtocol.ProtocolMessage>([
					...
					return dc
						...
						.then((response) => {
							...
							assert(
								...
								"Expected bp to not be verified yet"
							);
				...
				dc
					...
					.then((event: DebugProtocol.BreakpointEvent) => {
		...
		test("should stop on a breakpoint in source even if program's entry point is in JavaScript", () => {
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemaps-js-entrypoint/out/entry.js"
			);
			const OUT_DIR = path.join(
				DATA_ROOT,
				"sourcemaps-js-entrypoint/out"
			);
			const TS_SOURCE = path.join(
				DATA_ROOT,
				"sourcemaps-js-entrypoint/src/classes.ts"
			);
			const TS_LINE = 17;
		...
		test("can set a breakpoint in inlined sources", async () => {
			...
			await Promise.all([
				...
				dc.launch({ program: LAUNCH_PROGRAM, outFiles }),
				dc
					.assertStoppedLocation("debugger_statement", {
						path: PROGRAM,
						line: DEBUGGER_LINE,
					})
					.then((stackTrace) => {
			...
			const bpRequest = await dc.setBreakpointsRequest({
				breakpoints: [{ line: BP_LINE }],
				source: inlinedSource,
			});
			assert(
				bpRequest.body.breakpoints[0] &&
					bpRequest.body.breakpoints[0].verified
			);
		...
		test("still resolves sourcemap paths when they are absolute local paths", async () => {
			...
			const PROGRAM = path.join(
				DATA_ROOT,
				"sourcemaps-local-paths/out/classes.js"
			);
			const TS_SOURCE = path.join(
				DATA_ROOT,
				"sourcemaps-local-paths/src/classes.ts"
			);
			const TS_LINE = 17;
			...
			return dc.hitBreakpointUnverified(
				{
					program: PROGRAM,
					outFiles: [
						path.join(DATA_ROOT, "sourcemaps-local-paths/out/*.js"),
					],
					runtimeArgs: ["--nolazy"],
				},
				{ path: TS_SOURCE, line: TS_LINE }
	...
	suite("setExceptionBreakpoints", () => {
		...
		test("should not stop on exception in <node_internals> when skipFiles is used", async () => {
			const program = path.join(
				DATA_ROOT,
				"nodeInternalsCaughtException.js"
			);
		...
		test("should only show exception scope in the first frame", () => {
			...
			return Promise.all([
				dc
					.waitForEvent("initialized")
					.then((event) =>
						dc.setExceptionBreakpointsRequest({ filters: ["all"] })
					)
				...
				dc
					.assertStoppedLocation("exception", {
						path: PROGRAM,
						line: EXCEPTION_LINE,
					})
					.then(async (response) => {
						const frame0Id = response.body.stackFrames[0].id;
						const frame0Scopes = await dc.scopesRequest({
							frameId: frame0Id,
						});
						assert.equal(
							frame0Scopes.body.scopes[0].name,
							"Exception",
							"First frame should be named Exception"
						);

						const frame1Id = response.body.stackFrames[1].id;
						const frame1Scopes = await dc.scopesRequest({
							frameId: frame1Id,
						});
						assert.notEqual(
							frame1Scopes.body.scopes[0].name,
							"Exception",
							"First frame should not be named Exception"
						);
					}),
			]);
		});
	...
	suite("symlinks", () => {
		test("breakpoint in symlinked library module", () => {
			...
			const file2 = path.join(
				DATA_ROOT,
				"symlinked-file/symlinkToSrc/file.js"
			);
			const line = 1;
