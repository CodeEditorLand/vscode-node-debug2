...
suite("Node Debug Adapter etc", () => {
	...
	setup(() => {
		return testSetup.setup().then((_dc) => (dc = _dc));
	});
				...
				dc.on("output", (outputEvent) => {
					...
					if (msg.startsWith("log:") || msg.startsWith("error:")) {
						...
						if (type === lastEventType) {
							return reject(
								new Error(
									`Got two messages in a row of type ${type}`
								)
							);
						} else if (msg === "error: 9") {
							return resolve();
			...
			return new Promise((resolve, reject) => {
				...
				dc.on("terminated", () => {
					if (!gotOutput) {
						reject(
							new Error(
								"Terminated before exception output received"
							)
						);
					}
				});
	...
	suite("eval", () => {
		...
		test('returns "not available" for a reference error', () => {
			return start()
				.then(() =>
					dc.evaluateRequest({ expression: "notDefinedThing" })
				)
				.catch((response) => {
		...
		test("returns the error message for another error", () => {
			return start()
				.then(() =>
					dc.evaluateRequest({
						expression: 'throw new Error("fail")',
					})
				)
				.catch((response) => {
		...
		test("Shows array previews", () => {
			return start()
				.then(() =>
					dc.evaluateRequest({ expression: "[1, [1], {a: 3}]" })
				)
				.then((response) => {
					assert(response.success);
					assert(
						response.body.result ===
							"Array[3] [1, Array[1], Object]" ||
							response.body.result ===
								"Array(3) [1, Array(1), Object]"
					);
					assert(response.body.variablesReference > 0);
	...
	suite("completions", () => {
		...
		function testCompletions(
			...
			return start()
				...
				.then((stackTraceResponse) =>
					stackTraceResponse.body.stackFrames.map((frame) => frame.id)
				)
				.then((frameIds) =>
					dc.send("completions", <DebugProtocol.CompletionsArguments>{
						text,
						column,
						frameId: frameIds[frameIdx],
					})
				)
				.then(
		...
		function inCompletionsList(
			completions: DebugProtocol.CompletionItem[],
			...labels: string[]
		): boolean {
			return labels.every(
				(label) =>
					completions.filter((target) => target.label === label)
						.length === 1
			);
		}

		test("returns global vars", () => {
			return testCompletions("").then((completions) =>
		...
		test("returns from the correct frameId", () => {
			return testCompletions("obj", undefined, /*frameId=*/ 1).then(
				(completions) => assert(!inCompletionsList(completions, "obj"))
			);
		});
	...
	suite("hit condition bps", () => {
		...
		test("Does not bind when invalid", () => {
			...
			return Promise.all([
				ts.debugClient.setBreakpointOnStart(
					dc,
					bps,
					PROGRAM,
					undefined,
	...
	suite("get loaded scripts", () => {
		function assertHasSource(
			loadedSources: DebugProtocol.Source[],
			expectedPath: string
		): void {
			assert(
				loadedSources.find((source) => source.path === expectedPath)
			);
		}
		...
		test("returns all scripts", async () => {
			...
			// Has the eval script
			assert(
				sources.filter((source) => source.path.match(/VM\d+/)).length >=
					1
			);
		});
	});
	...
	suite("async callstacks", () => {
		function assertAsyncLabelCount(
			stackTrace: DebugProtocol.StackTraceResponse,
			expectedAsyncLabels: number
		): void {
			assert.equal(
				stackTrace.body.stackFrames.filter((frame) => !frame.source)
					.length,
				expectedAsyncLabels
			);
		}

		function assertStackFrame(
			stackTrace: DebugProtocol.StackTraceResponse,
			i: number,
			sourcePath: string,
			line: number
			...
			assert(
				frame.line === line || frame.line === line - 1,
		 ...
		 */
		function stripAsyncHookFrames(
			stackTrace: DebugProtocol.StackTraceResponse
		): DebugProtocol.StackFrame[] {
			return stackTrace.body.stackFrames.filter(
				(frame) =>
					!frame.source || frame.source.path.indexOf("async_hook") < 0
			);
		}
		...
		test("shows async stacks for promise resolution", async () => {
			...
			await dc.hitBreakpoint(
				{ program: PROGRAM, showAsyncStacks: true },
				{ path: PROGRAM, line: 45 }
			);
		...
		async function stepOverNativeAwait(fromLine: number, afterBp = false) {
			...
			if (
				utils.compareSemver(process.version, "v8.0.0") < 0 ||
				(utils.compareSemver(process.version, "v8.4.0") >= 0 &&
					utils.compareSemver(process.version, "v8.7.0") < 0)
			) {
		...
		test("shows async stacks and steps correctly for native async/await, pre v10", async () => {
			if (
				utils.compareSemver(process.version, "v7.6.0") < 0 ||
				utils.compareSemver(process.version, "v10.0.0") >= 0
			) {
				return Promise.resolve();
			}
