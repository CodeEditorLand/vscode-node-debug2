...
import {
	...
	IAttachRequestArguments,
	ICommonRequestArgs,
	ILaunchVSCodeArguments,
	ILaunchVSCodeArgument,
...
export class NodeDebugAdapter extends ChromeDebugAdapter {
	...
	public async launch(args: ILaunchRequestArguments): Promise<void> {
		...
		if (
			...
			typeof args._suppressConsoleOutput === "undefined"
		) {
		...
		const wslLaunchArgs = wsl.createLaunchArg(
			args.useWSL,
		...
		if (
			...
			await this.launchInInternalConsole(
				wslLaunchArgs.executable,
				wslLaunchArgs.args,
				envArgs,
				cwd
			);
	...
	private extensionHostLaunch(
		...
		return new Promise<void>((resolve, reject) => {
			this._session.sendRequest(
				"launchVSCode",
				launchVSCodeArgs,
				NodeDebugAdapter.RUNINTERMINAL_TIMEOUT,
				(response) => {
					if (response.success) {
						if (
							response.body &&
							typeof response.body.processId === "number"
						) {
	...
	public async attach(args: IAttachRequestArguments): Promise<void> {
		try {
			...
			if (
				err.format &&
				err.format.indexOf("Cannot connect to runtime process") >= 0
			) {
				// hack -core error msg
				err.format =
					"Ensure Node was launched with --inspect. " + err.format;
			}
	...
	protected commonArgs(args: ICommonRequestArgs): void {
		...
		args.smartStep =
			typeof args.smartStep === "undefined"
				? !this._isVSClient
				: args.smartStep;
	...
	protected async doAttach(
		...
		this._session.sendEvent(
			new CapabilitiesEvent({ supportsStepBack: this.supportsStepBack() })
		);
	}
	...
	private launchInInternalConsole(
		...
		return new Promise<void>((resolve, reject) => {
			...
			const noDebugMode = (<ILaunchRequestArguments>(
				this._launchAttachArgs
			)).noDebug;
	...
	private captureStderr(
		...
		nodeProcess.stderr.on("data", (data: string) => {
			...
			if (
				(this._handlingEarlyNodeMsgs ||
					noDebugMode ||
					this._captureFromStd) &&
				!this._launchAttachArgs._suppressConsoleOutput
			) {
	...
	protected onConsoleAPICalled(
		params: Crdp.Runtime.ConsoleAPICalledEvent
	): void {
		...
		if (
			...
			!params.args[0].value.match(NodeDebugAdapter.DEBUG_BRK_DEP_MSG)
		) {
	...
	public async terminateSession(
		...
		if (
			this.isExtensionHost() &&
			args &&
			typeof args.restart === "boolean" &&
			args.restart
		) {
			this.nodeProcessId = 0;
		...
		if (this._waitingForEntryPauseEvent) {
			...
			if (
				(this.normalAttachMode &&
					this._launchAttachArgs.stopOnEntry !== false) ||
	...
	private async resolveProgramPath(
		...
		if (this.jsDeterminant.isJavaScript(programPath)) {
			...
			if (generatedPath && generatedPath !== programPath) {
				...
				logger.log(
					`Launch: program '${programPath}' seems to be the generated file`
				);
			}
			...
			if (!sourceMaps) {
				return Promise.reject<string>(
					errors.cannotLaunchBecauseSourceMaps(programPath)
				);
			}
			...
			if (!generatedPath) {
				...
				if (
					this._launchAttachArgs.outFiles ||
					this._launchAttachArgs.outDir
				) {
					return Promise.reject<string>(
						errors.cannotLaunchBecauseJsNotFound(programPath)
					);
				} else {
					return Promise.reject<string>(
						errors.cannotLaunchBecauseOutFiles(programPath)
					);
				}
			}
	...
	private beginWaitingForDebuggerPaused(): void {
		...
		let count = this.normalAttachMode
			? 10
	...
	private async getNodeProcessDetailsIfNeeded(): Promise<void> {
		...
		if (response.exceptionDetails) {
            ...
            */
			telemetry.addCustomGlobalProperty({
				"Versions.Target.Version": version,
			});
		}
	}
	...
	protected globalEvaluate(
		args: Crdp.Runtime.EvaluateRequest
	 ...
	 */
	private getNotExistErrorResponse(
		attribute: string,
		path: string
	): Promise<void> {
		return Promise.reject(
	 ...
	 */
	private getRelativePathErrorResponse(
		attribute: string,
		path: string
	): Promise<void> {
		const format = localize(
	 ...
	 */
	private getErrorResponseWithInfoLink(
		code: number,
		format: string,
		variables: any,
		infoId: number
	...
	protected getReadonlyOrigin(aPath: string): string {
		return path.isAbsolute(aPath) ||
			aPath.startsWith(ChromeDebugAdapter.EVAL_NAME_PREFIX)
	...
	private isExtensionHost(): boolean {
		return (
			...
			this._adapterID === "legacy-extensionHost"
		);
	}
}
...
function detectSupportedDebugArgsForLaunch(
	config: ILaunchRequestArguments,
	runtimeExecutable: string,
	env: any
): DebugArgs {
	if (
		...
		return getSupportedDebugArgsForVersion(
			config.__nodeVersion || config.runtimeVersion
		);
